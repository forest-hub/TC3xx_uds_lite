/**********************************************************
**    File Name    	: multi_cyc_fifo.c
**    Description  	: manage multi cycy fifo
...
void WriteDataInFifo(uint16 i_xFifoId,unsigned char *i_pucWriteDataBuf, uint32 i_xWriteDatalen,tErroCode *o_peWriteStatus)
{
	...
	{
		(pstNode->pDataFifoAddr)[pstNode->xWriteAddr] = i_pucWriteDataBuf[xIndex];
...
{	
	...
	*o_pxReadLen = xCanReadTotal;
	...
	{
		o_pucReadDataBuf[xIndex] = (pstNode->pDataFifoAddr)[pstNode->xReadAddr] ;
