...
static tN_Result CANTP_DoTransmitFC(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
{
    uint8 aucTransDataBuf[STANDARD_CAN_DL] = {0u};
    ...
    if(TRUE == CANTP_IsEnableTxCANFDMsg())
    {
        CANType = CANTP_FD;
    }
    else
    {
        CANType = CANTP_STANDARD;
    }

    /*set frame type*/
    (void)CANTP_SetFrameType(CANType, FC, &aucTransDataBuf[0u]);
...
static tN_Result CANTP_DoTransmitSF(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
{
    ...
    (void)CANTP_SetFrameType(CANType, SF, &aDataBuf[0u]);
    ...
    if(TRUE != CANTP_FillTxMsgInfo(SF,
                        gs_stCanTPTxDataInfo.stCanTpDataInfo.xFFDataLen,
                        gs_stCanTPTxDataInfo.stCanTpDataInfo.aDataBuf,
                        DATA_LEN,
                        aDataBuf,
...
static tN_Result CANTP_DoTransmitFF(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
{
    ...
    /*set transmitted frame type*/
    (void)CANTP_SetFrameType(CANType, FF, &aDataBuf[0u]);
    ...
    if(TRUE != CANTP_FillTxMsgInfo(FF,
                        gs_stCanTPTxDataInfo.stCanTpDataInfo.xFFDataLen,
                        gs_stCanTPTxDataInfo.stCanTpDataInfo.aDataBuf,
                        DATA_LEN,
                        aDataBuf,
...
static tN_Result CANTP_DoTransmitCF(tCanTpMsg * m_stMsgInfo, tCanTpWorkStatus *m_peNextStatus)
{
    ...
    (void)CANTP_SetFrameType(CANType, CF, &aTxDataBuf[0u]);
    ...
    if(txLen >= TX_CF_DATA_MAX_LEN)
    {
        ...
        AddTxDataLen(TX_CF_DATA_MAX_LEN);
    }
                                ...
                                uint8 *o_pFrameType)
{
    ...
    if(SF == i_eFrameType ||
       ...
       CF == i_eFrameType)
    {
        if((CANTP_FD == i_eCANType) && (SF == i_eFrameType))
        {
            *o_pFrameType = (uint8)SF;
        }
        else
        {
            *o_pFrameType &= 0x0Fu;

            *o_pFrameType |= ((uint8)i_eFrameType << 4u);
        }
...
static boolean CANTP_SetTxSFDataLen(const tCANType i_eCANType, const uint32 i_txSFDataLen, uint8 *o_pSFMsgBuf)
{
    ...
    if(i_txSFDataLen <= SF_CAN_DATA_MAX_LEN)
    {
        *o_pSFMsgBuf &= 0xF0u;
        *o_pSFMsgBuf |= (uint8)i_txSFDataLen;

        res = TRUE;
    }
    else
    {
        /*only CAN FD tx msg can over SF_CAN_DATA_MAX_LEN*/
        if((CANTP_FD == i_eCANType) && (i_txSFDataLen <= SF_CANFD_DATA_MAX_LEN))
        {
            o_pSFMsgBuf[1u] = (uint8)i_txSFDataLen;
...
static boolean CANTP_SetTxFFDataLen(const uint32 i_txSFDataLen, uint8 *o_pFFMsgBuf)
{
    if((i_txSFDataLen <= SF_CAN_DATA_MAX_LEN) || (NULL_PTR == o_pFFMsgBuf))
    {
        return FALSE;
    }
    ...
    *(o_pFFMsgBuf + 0u) &= 0xF0u;
    *(o_pFFMsgBuf + 0u) |= (uint8)((i_txSFDataLen) >> 8u);
    *(o_pFFMsgBuf + 1u) |= (uint8)(i_txSFDataLen);

    return TRUE;
}

/*Is Frame type valid?*/
static boolean CANTP_IsFrameTypeValid(const tNetWorkFrameType i_eFrameType)
{
    if(SF == i_eFrameType ||
       FF == i_eFrameType ||
       FC == i_eFrameType ||
...
#ifdef EN_TX_CAN_FD /*CAN FD message*/
    res = TRUE;
#else
    res = FALSE;
#endif
                                     ...
                                     uint32 *o_pMaxDataLen)
{
    boolean res = FALSE;
    uint8 dataStartPos = 0u;
    uint32 maxDataLen = 0u;
    boolean isCANFDType = FALSE;

    if((0u == i_fillMsgLen) || (NULL_PTR == o_pDataStartPos) || (NULL_PTR == o_pMaxDataLen))
    {
        return FALSE;
    }
    ...
    /*CAN standard*/
    if(CANTP_STANDARD == i_CANType)
    {
        isCANFDType = FALSE;
    }
    else if(CANTP_FD == i_CANType)
    {
        /*CAN FD*/
        isCANFDType = TRUE;
    }
    ...
    if(SF == i_eFrameType)
    {
        if(i_fillMsgLen < STANDARD_CAN_DL)
        {
            dataStartPos = 1u;
        }
        else
        ...
        /*SF data len*/
        maxDataLen = TX_SF_DATA_MAX_LEN;
    }
    else if(CF == i_eFrameType)
    {
        /*CAN and CAN FD are the same data start position*/
        dataStartPos = 1u;

        /*CF data len*/
        maxDataLen = TX_CF_DATA_MAX_LEN;
    }
    else if(FF == i_eFrameType)
    {
        /*CAN and CAN are the same data start position*/
        dataStartPos = 2u;

        if(TRUE == isCANFDType)
        {
            /*CAN FD FF is max 62 bytes, but FF is less than 63 bytes*/
            maxDataLen = TX_FF_DATA_MIN_LEN - 1u;
        }
        else
        {
            /*standard CAN FF is max 6 bytes, but FF is less than 8 bytes*/
            maxDataLen = TX_FF_DATA_MIN_LEN - 2u;
        }
    }
    else if(FC == i_eFrameType)
    {
        /*CAN and CAN FD have the same data start position and data length*/
        dataStartPos = 1u;
    ...
    if(TRUE == res)
    {
        *o_pDataStartPos = dataStartPos;
        *o_pMaxDataLen = maxDataLen;
    }
                                   ...
                                   uint32 *o_pDstMsgLen)
{
    boolean res = FALSE;
    uint8 dataStartPos = 0u;
    uint32 maxDataMsgLen = 0u;
    tCANType CANType = CANTP_STANDARD;
    uint32 txMsgLen = 0u;

    if((0 == i_srcMsgLen) ||
       (NULL_PTR == i_pSrcMsgBuf) ||
       /*(i_maxDstMsgBufLen <= i_srcMsgLen) || */
       (NULL_PTR == o_pDstMsgLen))
    {
        return FALSE;
    }

    if(TRUE != CANTP_IsFrameTypeValid(i_eFrameType))
    {
        return FALSE;
    }
    ...
    res = CANTP_GetFillDataInfo(CANType, i_eFrameType, i_srcMsgLen, &dataStartPos, &maxDataMsgLen);
    if(TRUE == res)
    {
        res = CANTP_SetFrameType(CANType, i_eFrameType, o_pDstMsgBuf);
        if(TRUE == res)
        {
            /*for SF and FF need to fill message length*/
            if(SF == i_eFrameType)
            {
                res = CANTP_SetTxSFDataLen(CANType, i_srcMsgLen, o_pDstMsgBuf);
                txMsgLen = (uint32)i_srcMsgLen;
            }
            else if(FF == i_eFrameType)
            {
                res = CANTP_SetTxFFDataLen(i_srcMsgLen, o_pDstMsgBuf);
                txMsgLen = (uint32)maxDataMsgLen;
            }
            else if(CF == i_eFrameType)
            {
                /*frame is CF don't fill message length*/
                txMsgLen = ((uint32)i_srcMsgLen > maxDataMsgLen) ? (maxDataMsgLen) : (uint32)i_srcMsgLen;
...
static boolean GetRxSFMsgValidPosition(const uint32 i_RxMsgLen, uint8* o_pDataStartPos)
{
    if((NULL_PTR == o_pDataStartPos) || (0u == i_RxMsgLen))
    {
        return FALSE;
    }

    /*data Bytes <= 8, rx stanard CAN message*/
    if(i_RxMsgLen <= STANDARD_CAN_DL)
    {
        *o_pDataStartPos = 1u;
    }
    ...
    {
        /*data Bytes > 8, rx message is CAN FD*/
        *o_pDataStartPos = 2u;
    }
...
static boolean GetRXFFFrameMsgLength(const uint32 i_RxMsgLen, const uint8 *i_pMsgBuf, uint32 *o_pFrameLen)
{
    ...
    if((i_RxMsgLen < STANDARD_CAN_DL) || (TRUE != IsFF(i_pMsgBuf[0u])))
    {
        return FALSE;
    }
    ...
    if(frameLen < RX_FF_DATA_LESS_LEN)
    {
        result = FALSE;
    }
    else
